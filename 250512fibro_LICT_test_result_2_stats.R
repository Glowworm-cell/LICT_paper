...
df4 = read.csv('./avg_log2FC_step1_res1.csv')
df3 = read_excel('./pn_gene.xlsx')
df = read.csv('../embryo2/LICT_result.csv')
df = df[,36:48]
for (i in 1:nrow(df1)) {
  n = -1
  for (j in c(3, 8, 13, 18, 23)) {  # 确保圆括号完整闭合
    n = n + 2
    if (grepl(df4$cluster[i], df1[i, j], ignore.case = TRUE) ||
        grepl(df1[i, j], df4$cluster[i], ignore.case = TRUE)) {
      df[i, n] = 1
      df[i,n+1] = df1[i, j]
    } else {
      df[i, n] = 0
...
df$max_score = mapply(max, df$Gemini_score, df$Llama_score,df$ERNIE_score,df$ChatGPT_score,df$Claude_score)
for (i in 1:nrow(df1)){
  if (df[i,]$max_score == 1){
    df[i,]$max_match = df4$cluster[i]
  }else{
    df[i,]$max_match = 'no match'
  }
}
df$Free_LLMs_score = mapply(max, df$Gemini_score, df$Llama_score)
df1 = cbind(df1,df)
...
df1 = read.csv('./LICT_feedback_result.csv')
colnames(df1)[3] = 'ERNIE_annotation'
colnames(df1)[8] = 'Gemini_annotation'
colnames(df1)[13] = 'GPT_annotation'
colnames(df1)[18] = 'Llama_annotation'
...
for(i in 1:nrow(LICT_res)){
  if(LICT_res[i,]$Total_relialbe == 'YES'){
...
###row2
for(i in 1:nrow(LICT_res)){
  if(LICT_res[i,]$Gemini_relialbe == 'YES' | LICT_res[i,]$Llama_relialbe == 'YES'){
    if(LICT_res[i,]$manual_reliable == 'YES'){
...
for(i in 1:nrow(LICT_feedback_res)){
  if(LICT_feedback_res[i,]$Total_relialbe == 'YES'){
...
###row4
for(i in 1:nrow(LICT_feedback_res)){
  if(LICT_feedback_res[i,]$Gemini_relialbe == 'YES' | LICT_feedback_res[i,]$Llama_relialbe == 'YES'){
    if(LICT_feedback_res[i,]$manual_reliable == 'YES'){
...
for(i in 1:nrow(mismatch_LICT_res)){
  if(mismatch_LICT_res[i,]$Total_relialbe == 'YES'){
...
###row2
for(i in 1:nrow(mismatch_LICT_res)){
  if(mismatch_LICT_res[i,]$Gemini_relialbe == 'YES' | mismatch_LICT_res[i,]$Llama_relialbe == 'YES'){
    if(mismatch_LICT_res[i,]$manual_reliable == 'YES'){
...
for(i in 1:nrow(mismatch_LICT_feedback_res)){
  if(mismatch_LICT_feedback_res[i,]$Total_relialbe == 'YES'){
...
###row4
for(i in 1:nrow(mismatch_LICT_feedback_res)){
  if(mismatch_LICT_feedback_res[i,]$Gemini_relialbe == 'YES' | mismatch_LICT_feedback_res[i,]$Llama_relialbe == 'YES'){
    if(mismatch_LICT_feedback_res[i,]$manual_reliable == 'YES'){
