setwd('D:/R/LLMsCellType/240615pbmc_different_method4/')
detach(package:LLMcellidentifier, unload=TRUE)
install.packages("D:/ywj/LLM514/LICT_0.1.0.tar.gz", repos = NULL, type = "source")
library(LICT)
library(reticulate)
library(rols)
library(dplyr)
...
")
markers = readRDS('./pbmc_markers_subtype.rds')
top_markers <- markers %>% dplyr::group_by(cluster) %>% dplyr::top_n(n = 10, wt = avg_log2FC)
pbmc_res = LICT::LLMCellType(FindAllMarkersResult = markers,
                       species = 'human',
                       topgenenumber = 10,
                       tissuename = 'peripheral blood mononuclear cell')
pbmc_res_GPT1 = GPTCellType(input = top_markers[1:150,],
                                  species = 'human',
                                  topgenenumber = 10,
                                  tissuename = 'peripheral blood mononuclear cell')
pbmc_res_GPT2 = GPTCellType(input = top_markers[150:310,],
                            species = 'human',
                            topgenenumber = 10,
                            tissuename = 'peripheral blood mononuclear cell')
pbmc_res_gemini = Gemini_input(input = top_markers[150:310,],
                            species = 'human',
                            topgenenumber = 10,
                            tissuename = 'peripheral blood mononuclear cell')
pbmc_res_GPT = rbind(pbmc_res_GPT1, pbmc_res_GPT2)
pbmc_res[['GPT']] = pbmc_res_GPT
E = c('1: Monocyte\n2: T cell\n3: CD8+ T cell\n4: NK cell\n5: Cytotoxic T lymphocyte\n6: Macrophage\n7: B cell\n8: Naive T cell\n9: Cytotoxic T lymphocyte\n10: B cell\n11: NKT cell\n12: NKT cell\n13: Th2 cell\n14: undefined\n15: Plasma cell\n16: Megakaryocyte\n17: B cell\n18: Cell in mitosis\n19: Regulatory T cell\n20: Plasma cell\n21: NKT cell\n22: Dendritic cell\n23: Cell in mitosis\n24: Erythroblast\n25: undefined\n26: Megakaryocyte\n27: NKT cell\n28: Cell in mitosis\n29: Erythrocyte\n30: Dendritic cell\n31: undefined')
rows <- strsplit(E, "\n")[[1]]
...
res1[2,1] = 'CD4 T Central Memory'
res1[3,1] = 'CD8 Naive T'
res1[4,1] = 'Natural Killer cells'
res1[5,1] = 'CD8 T Effector Memory'
res1[9,1] = 'CD4 cytotoxic T cells'
res1[10,1] = 'B naive'
res1[11,1] = 'mucosal associated invariant T'
res1[12,1] = 'gamma delta t'
res1[13,1] = 'CD8 T Central Memory'
res1[14,1] = 'double negative T'
res1[17,1] = 'plasmacytoid DC'
res1[21,1] = 'CD4 T Effector Memory'
res1[22,1] = 'conventional dendritic cells2'
res1[24,1] = 'AS dendritic cells'
res1[25,1] = 'Hematopoietic Stem and Progenitor Cell'
res1[27,1] = 'NK cells into CD56-bright'
res1[29,1] = 'erythrocyte'
res1[30,1] = 'conventional dendritic cells1'
res1[31,1] = 'innate lymphoid cells'
...
res1 = read.csv('./avg_log2FC_res_step2_v1.csv')
...
#########1
inter_res_1 = LLM_interect(positive_gene = list(
  row1 = c('CD36','SERPINA1','AC020656.1','MPEG1','FPR1','CD14','S100A8','S100A9','LYZ','FCN1'),
  row2 = c('CD40LG','TNFRSF4','TRADD','RCAN3','TNFRSF25','CD3D','CD3E','CD3G','CD2','TRAC','TRBC2'),
  row3 = c(),
  row4 = c(),
  row5 = c('FGFBP2','GNLY','CTSW','GZMB','PRF1','CD8A','CD3E','CD3D','CD2','NKG7'),
  row6 = c('HMOX1','SIGLEC10','CTSL','C1QA','LST1','CD68','FCGR3A','FCGR3A','CSF1R','S100A9','LYZ'),
  row7 = c('IGHD','IGLC3','LINC00926','CD22','TCF4','MS4A1','MS4A1','CD79A','CD79B','BANK1'),
  row8 = c('LINC00861','NOSIP','OXNAD1','CAMK4','BCL11B','SELL','CD3E','CD3D','CD3G','TRAC','TRBC2'),
  row9 = c('CD2','MAF','KLRG1','C12orf75','TGFBR3','NKG7','GNLY'),
  row10 = c('VPREB3','CD22','TNFRSF13C','IGLC2','FCER2','MS4A1','MS4A1','CD79A','CD79B','BANK1'),
  row11 = c('TRGC2','LYAR','CD69','CCL5','MYBL1','CD3E','CD3D','CD3G','KLRB1'),
  row12 = c('GZMA','GZMH','DUSP2','GZMK','CTSW','CD3E','CD3D','CD3G'),
  row13 = c('GPR171','TCF7','CLDND1','RCAN3','CD3D','CD3E','CD3D','CD2'),
  row14 = c('TIGIT','MYB','TTN','LEF1','SESN3','CD3E','CD3D','CD3G'),
  row15 = c('AIM2','LINC00926','VPREB3','CD79B','JCHAIN','CD79A','MS4A1','MS4A1','CD79B','BANK1'),
  row16 = c('TCF7L2','HES4','RGS18','IFITM3','CFD','S100A8','S100A9'),
  row17 = c(),
  row18 = c('PTTG1','CENPF','SMC2','NUSAP1','CHEK1','PTPRC','VIM'),
  row19 = c(),
  row20 = c('TXNDC5','SEC11C','IGLC3','PDIA4','CD38','MZB1','CD79A','JCHAIN','CD38'),
  row21 = c('CD40LG','SYNE2','PDE4D','LTB','DPP4','CD3E'),
  row22 = c('HLA-DMB','LGALS2','HLA-DPB1','CST3','PLD4','CD1C'),
  row23 = c('HELLS','RRM2','ATAD2','MCM4','TPX2','PTPRC','VIM'),
  row24 = c('IRF8','CCDC50','SCT','SOX4','DAB2'),
  row25 = c('EGFL7','GATA2','STMN1','NPR3','CD34'),
  row26 = c(),
  row27 = c('NCAM1','TRDC','CTSW','SPTSSB','HOPX','XCL2','CD7','CD2'),
  row28 = c('TPX2','BIRC5','TK1','HMMR','CLSPN','PTPRC','VIM'),
  row29 = c(),
  row30 = c('SHTN1','BASP1','CPVL','FLT3','TSPAN33','FLT3')),
  negative_gene = list(
    row1 = c('FCGR3A','CCR2','ITGAM','FCGR3A','CSF1R','MPO','ELANE','FCGR1A','CXCR2'),
    row2 = c('CD5','CD7','CD28','CD6','TRBC1','MS4A1','CD19','MS4A1','SELL'),
    row3 = c(),
    row4 = c(),
    row5 = c('CD8B','GZMB','PRF1','CD27','CD28','CCR5','FCGR3A'),
    row6 = c('FCGR1A','CD14','ITGAM','ITGAX','MRC1','S100A8','CCR2','MARCO','MSR1'),
    row7 = c('CD19','CD22','CD24','CD40','PAX5','BLNK','BTK','MALT1'),
    row8 = c('CCR7','CD2','CD5','CD7','CD28','CD6','TRBC1','LCK'),
    row9 = c('GZMB','PRF1','KIR2DL1','KIR3DL1','FCGR3A','CD8A','CD8B','KLRD1','KLRC1','KLRK1','HAVCR2'),
    row10 = c('CD19','CD22','CD24','CD40','PAX5','BLNK','BTK','MALT1'),
    row11 = c('NCAM1','CD2','CD7','CD28','CD5','CD1D','KIR2DL1','KIR3DL1','XCL1','XCL2'),
    row12 = c('NCAM1','CD2','CD7','CD28','CD5','CD1D','KLRB1','KIR2DL1','KIR3DL1','CD8A','CD8B'),
    row13 = c('GATA3','IL4','CD4','CD5','CD28','CD8A','CD8B','GZMB','PRF1'),
    row14 = c('CD2','CD5','CD7','CD28','CD6','CCR7','SELL','FCGR3A'),
    row15 = c('MZB1','POU5F1','JCHAIN','CD38','IL2RA','CD19','PAX5','CD22'),
    row16 = c('PF4','CD14','FCGR3A','FCGR1A','MZB1'),
    row17 = c(),
    row18 = c(''),
    row19 = c(),
    row20 = c('SDC1','POU5F1','IL2RA','FOXP3','CTLA4','CD4','CD5'),
    row21 = c('NCAM1','CD1D','KLRB1','MZB1','POU5F1','CCR7','SELL','KIR2DL1','KIR3DL1','FCGR3A'),
    row22 = c('S100B','CCR7','CD8A','CD28','CD27'),
    row23 = c('NANOG','CD1C','S100B'),
    row24 = c('HBB','MZB1','CD79A','JCHAIN','CD38','CD19'),
    row25 = c('CD34','CD38','GATA2','TAL1','CD44','FLT3'),
    row26 = c(),
    row27 = c('NCAM1','CD1D','KLRB1','CD3E','KIR2DL1','KIR3DL1','FCGR3A','XCL1','CD5','CD28'),
    row28 = c('MS4A1','MS4A1','CD79A','CD79B','CD22'),
    row29 = c(),
    row30 = c('CD1C','S100B','CD40','CD80','CD86','CCR7','CD83')))
E = c('1: Classical monocytes\n2: Naive CD4+ T cells\n3: Naive CD8+ T cells\n4: NK cells\n5: CD8+ T cells\n6: Non-classical monocytes\n7: Naive B cells\n8: Central memory CD4+ T cells\n9: CD56bright NK cells\n10: Memory B cells\n11: CD56dim CD16+ NK cells\n12: NK cells\n13: Naive CD8+ T cells\n14: Central memory CD4+ T cells\n15: Plasmablasts\n16: Megakaryocytes\n17: Plasmacytoid dendritic cells\n18: Proliferating cells\n19: Regulatory T cells\n20: Plasma cells\n21: Naive CD4+ T cells\n22: Myeloid dendritic cells\n23: Proliferating cells\n24: Plasmacytoid dendritic cells\n25: Hematopoietic stem cells\n26: Platelets\n27: CD56bright CD16- NK cells\n28: Proliferating cells\n29: Erythrocytes\n30: Plasmacytoid dendritic cells\n31: Innate lymphoid cells')
rows <- strsplit(E, "\n")[[1]]
data <- sapply(rows, function(row) strsplit(row, ": ")[[1]])
df1 <- data.frame(clusters = as.integer(gsub(">", "", data[1, ]))-1, cell_type = data[2, ])
inter_res_1[['Claude']] = df1
res = inter_res_1

cluster_genes = readRDS('./pbmc_cluster_genes_inter1.rds')
res1 = create_dataframe(res, markers)
res1$cluster = as.character(res1$cluster)
res1[2,1] = 'CD4 T Central Memory'
res1[3,1] = 'CD8 Naive T'
...
ggplot(score_distribution, aes(x = Model, y = Percentage, fill = factor(Score))) +
  ...
  geom_text(aes(label = paste0(round(Percentage, 1), "%")), position = position_stack(vjust = 0.5)) +
  scale_y_continuous(labels = percent_format(scale = 1), position = "right") +
  labs(x = "Model", y = "Percentage", fill = "Score", title = "PBMC Evaluation Scores") +
  theme_bw() +
  scale_fill_manual(values = coldf$color) +
  theme(axis.text.x = element_text(angle = 0, hjust = 0.5)) +
  coord_flip()
...
ggplot(score_distribution, aes(x = Model, y = Percentage, fill = factor(Score))) +
  geom_bar(stat = "identity") +
  scale_y_continuous(labels = scales::percent_format(scale = 1),position = "right") +
  labs(x = "Model", y = "Percentage", fill = "Score", title = "PBMC")+
  theme_bw()+
  scale_fill_manual(values = coldf$color) +
  theme(axis.text.x = element_text(angle = 0, hjust = 0.5))+
  coord_flip()
