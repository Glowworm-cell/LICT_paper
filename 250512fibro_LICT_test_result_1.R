...
ANTHROPIC_API_KEY = 'sk-vpTHbZtpM6mebucT1dCb54530c6546De95C5Be50376c1d61'
")
...
markers = readRDS('../../LLMsCellType/markers/Mouse_SS_Fibro_markers.rds')
####loading package
library(LLMTissueIdentifier)
####input FindAllMarkers() function result, species, top gene number, and celltype
LLMCelltype_res = LLMtissuetype(FindAllMarkersResult = markers,
                              species = 'mouse',
                              topgenenumber = 10,
                              celltype = 'fibroblast')
#save result
saveRDS(LLMCelltype_res,'./LLMCelltype_res.rds')
LLMCelltype_res = readRDS('./LLMCelltype_res.rds')
####input LLMCelltype() function result, seurat_obj, and Percent
Validate_res = Validate(LLM_res = LLMCelltype_res, seurat_obj = seurat_obj, Percent = 0.8)
Reliable_Df = Reliable_Df(Validate_res)
Reliable_Df$Clusters = seq(0,nrow(Reliable_Df)-1)
Reliable_Df = Reliable_Df[,c(7,1:6)]
##del
Reliable_Df = readRDS('./Reliable_Df.rds')
##
Reliable_Df
##del
Validate_res = readRDS('./Validate_res.rds')
##
Validate_Result_to_Df = Validate_Result_to_Df(Validate_res)
##save LICT_result
LICT_result = cbind(Validate_Result_to_Df, Reliable_Df)
write.csv(LICT_result,'./LICT_result.csv')
...
#save result
saveRDS(interacted_res,'./interacted_res.rds')
interacted_res = readRDS('./interacted_res.rds')
##
feedback_Validate = Validate(LLM_res = interacted_res, seurat_obj = seurat_obj, Percent = 0.6)
feedback_Validate = readRDS('./feedback_Validate.rds')
...
write.csv(LICT_feedback_result,'./LICT_feedback_result.csv')
...
def get_class_id_from_ontology(path, label):
    try:
        # 加载本体
        ...
        # 搜索具有指定标签的所有类
        results = onto.search(label=label)
        if results:
            # 返回第一个匹配的类的ID
        ...
        print('Failed to load the ontology or find the class:', e)
        raise
              ")
cl = res1[,1:2]
...
cluster_genes = readRDS('../gc2/gc_cluster_genes_inter1.rds')
res1 = create_dataframe(res, markers)
res1$cluster = as.character(res1$cluster)
res1[11,1] = 'B cells'
res1[2,1] = 'Epithelial cells'
