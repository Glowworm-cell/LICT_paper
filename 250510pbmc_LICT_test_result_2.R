...
Idents(seurat_obj) = seurat_obj$celltype.l2
markers <- FindAllMarkers(object = seurat_obj, only.pos = T, min.pct = 0.25, logfc.threshold = 0.25)
###here seurat object had 31 clusters, we split it to 2 parts
seurat_obj1 = subset(seurat_obj, celltype.l2 %in% levels(Idents(seurat_obj))[1:16])
seurat_obj2 = subset(seurat_obj, celltype.l2 %in% levels(Idents(seurat_obj))[17:31])
markers1 = subset(markers, cluster %in% levels(Idents(seurat_obj1)))
markers2 = subset(markers, cluster %in% levels(Idents(seurat_obj2)))
saveRDS(markers1,'./pbmc_markers1.rds')
saveRDS(markers2,'./pbmc_markers2.rds')
markers = readRDS('../../LLMsCellType/240615pbmc_different_method4/pbmc_markers_subtype.rds')
markers1 = readRDS('./pbmc_markers1.rds')
markers2 = readRDS('./pbmc_markers2.rds')
####loading package
library(LICT)
...
# 计算差异，并转换为毫秒
execution_time_ms <- (end_time - start_time)[3] * 1000  # 获取elapsed时间并转换为毫秒

# 输出总共用时（毫秒）
cat("总共用时：", execution_time_ms, "毫秒\n")

LLMCelltype_res = LLMCellType(FindAllMarkersResult = markers1,
                              species = 'human',
                              topgenenumber = 10,
                              tissuename = 'peripheral blood mononuclear cell')

#save result1
saveRDS(LLMCelltype_res,'./LLMCelltype_res_1.rds')
####input LLMCelltype() function result, seurat_obj, and Percent
Validate_res = Validate(LLM_res = LLMCelltype_res, seurat_obj = seurat_obj1, Percent = 0.6)
Reliable_Df = Reliable_Df(Validate_res)
Reliable_Df$Clusters = seq(0,nrow(Reliable_Df)-1)
Reliable_Df = Reliable_Df[,c(7,1:6)]
Reliable_Df
saveRDS(Validate_res,'./Validate_res_1.rds')
saveRDS(Reliable_Df,'./Reliable_Df_1.rds')
Reliable_Df = readRDS('./Reliable_Df_1.rds')
Validate_res = readRDS('./Validate_res_1.rds')
##
Validate_Result_to_Df = Validate_Result_to_Df(Validate_res)
##save LICT_result
LICT_result = cbind(Validate_Result_to_Df, Reliable_Df)
write.csv(LICT_result,'./LICT_result_1.csv')
...
interacted_res = Feedback_Info(Validate_Result_to_Df, 11, 20, markers1)

#save result
saveRDS(interacted_res,'./interacted_res_1.rds')
##
feedback_Validate = Validate(LLM_res = interacted_res, seurat_obj = seurat_obj1, Percent = 0.6)
feedback_Validate = readRDS('./feedback_Validate_1.rds')
...
saveRDS(interacted_res,'./interacted_res_1.rds')
##
feedback_Validate_Result_to_Df = Validate_Result_to_Df(Validate_res)
##save LICT_result
LICT_feedback_result = cbind(feedback_Validate_Result_to_Df, intersect_res)
write.csv(LICT_feedback_result,'./LICT_feedback_result_1.csv')



####input FindAllMarkers() function result, species, top gene number, and tissuename
LLMCelltype_res = LLMCellType(FindAllMarkersResult = markers2,
                              species = 'human',
                              topgenenumber = 10,
                              tissuename = 'peripheral blood mononuclear cell')
#save result1
saveRDS(LLMCelltype_res,'./LLMCelltype_res_2.rds')
####input LLMCelltype() function result, seurat_obj, and Percent
Validate_res = Validate(LLM_res = LLMCelltype_res, seurat_obj = seurat_obj2, Percent = 0.6)
Reliable_Df = Reliable_Df(Validate_res)
Reliable_Df$Clusters = seq(0,nrow(Reliable_Df)-1)
Reliable_Df = Reliable_Df[,c(7,1:6)]
Reliable_Df
saveRDS(Validate_res,'./Validate_res_2.rds')
saveRDS(Reliable_Df,'./Reliable_Df_2.rds')
Reliable_Df = readRDS('./Reliable_Df_2.rds')
Validate_res = readRDS('./Validate_res_2.rds')
##
Validate_Result_to_Df = Validate_Result_to_Df(Validate_res)
##save LICT_result
LICT_result = cbind(Validate_Result_to_Df, Reliable_Df)
write.csv(LICT_result,'./LICT_result_2.csv')
####input Validate_Result_to_Df() result and you want to put top how many FindAllMarkers() result next top DEGs to LLMs. Here we use top 11 to 20 DEGs.
interacted_res = Feedback_Info(Validate_Result_to_Df, 11, 20, markers2)
#save result
saveRDS(interacted_res,'./interacted_res_2.rds')
##
feedback_Validate = Validate(LLM_res = interacted_res, seurat_obj = seurat_obj, Percent = 0.6)
feedback_Validate = readRDS('./feedback_Validate_2.rds')
Feedback_Reliable_Df = Reliable_Df(feedback_Validate)
Feedback_Reliable_Df$Clusters = seq(0,nrow(Feedback_Reliable_Df)-1)
Feedback_Reliable_Df = Feedback_Reliable_Df[,c(7,1:6)]
intersect_res = intersect_result(Reliable_Df,Feedback_Reliable_Df)
...
##save LICT_result
LICT_feedback_result = cbind(feedback_Validate_Result_to_Df, intersect_res)
write.csv(LICT_feedback_result,'./LICT_feedback_result_2.csv')
...
n1 = readRDS('./LLMCelltype_res_1.rds')
n2 = readRDS('./LLMCelltype_res_2.rds')
res = n1
res$ERNIE = rbind(n1$ERNIE,n2$ERNIE)
res$Gemini = rbind(n1$Gemini,n2$Gemini)
res$GPT = rbind(n1$GPT,n2$GPT)
res$Llama = rbind(n1$Llama,n2$Llama)
res$Claude = rbind(n1$Claude,n2$Claude)
...
n1 = readRDS('./interacted_res_1.rds')
n2 = readRDS('./interacted_res_2.rds')
res = n1
res$ERNIE = rbind(n1$ERNIE,n2$ERNIE)
res$Gemini = rbind(n1$Gemini,n2$Gemini)
res$GPT = rbind(n1$GPT,n2$GPT)
