...
score[score == 2] = 0.5
score[score == 3] = 1
score = score[c("max_score", "free_max_score")]
df2 = score
colnames(df2) = c("max_score_2", "free_max_score")
df = cbind(df1$ChatGPT_score,df2)
pbmc = colMeans(df)
score = df
...
ggplot(score_distribution, aes( x = Model, y=Percentage, fill = factor(Score),
  ...
  geom_flow(width = 0.65, alpha = 0.3, knot.pos = 0.3,
            color = '#8e8e8e', size = 1.5)+
  coord_flip()+
...
ggsave(
  filename = './fig/figure6_2/pbmc1.pdf', # 保存的文件名称。通过后缀来决定生成什么格式的图片
  width = 8,             # 宽
  height = 4,            # 高
  units = "in",          # 单位
  dpi = 100)
...
ggplot(score_distribution, aes( x = Model, y=Percentage, fill = factor(Score),
  ...
  geom_col(width = 0.65,
           color = '#8e8e8e', size = 1.5) +
  geom_flow(width = 0.65, alpha = 0.30, knot.pos = 0.3,
            color = '#8e8e8e', size = 1.5)+
  coord_flip()+
  scale_fill_manual(values = c("#efefef","#83dbca","#12846e"))+
  scale_y_continuous(expand = c(0,0),name="",
...
ggsave(
  filename = './fig/figure6_2/gc1.pdf', # 保存的文件名称。通过后缀来决定生成什么格式的图片
  width = 8,             # 宽
  height = 4,            # 高
  units = "in",          # 单位
  dpi = 100)
...
ggsave(
  filename = './fig/figure6_2/embyro1.pdf', # 保存的文件名称。通过后缀来决定生成什么格式的图片
  width = 8,             # 宽
  height = 4,            # 高
  units = "in",          # 单位
  dpi = 100)
...
ggplot(score_distribution, aes( x = Model, y=Percentage, fill = factor(Score),
  ...
  geom_flow(width = 0.65, alpha = 0.30, knot.pos = 0.3,
            color = '#8e8e8e', size = 1.5)+
  coord_flip()+
  scale_fill_manual(values = c("#efefef","#12846e"))+
  scale_y_continuous(expand = c(0,0),name="",
                     label=c("0%","25%","50%","75%","100%"))+
  scale_x_discrete(expand = c(0,0),name="")+
  theme(panel.background = element_blank(),
...
ggsave(
  filename = './fig/figure6_2/fibro1.pdf', # 保存的文件名称。通过后缀来决定生成什么格式的图片
  width = 8,             # 宽
  height = 4,            # 高
  units = "in",          # 单位
  dpi = 100)
...
long_data$model = factor(long_data$model, levels = c('df1$ChatGPT_score',
                                                     'free_max_score','max_score_2'))
long_data$tissue = factor(long_data$tissue, levels = c('fibro','embryo','gc',
                                                       'pbmc'))
heatmap_plot <- ggplot(long_data, aes(x = tissue, y = model, fill = score)) +
  ...
  labs(x = "Model", y = "Data Type", fill = "Score")+
  coord_flip()# 添加标签
heatmap_plot
ggsave(
  filename = './fig/figure6_2/heatmap.pdf', # 保存的文件名称。通过后缀来决定生成什么格式的图片
  width = 4,             # 宽
  height = 5,            # 高
  units = "in",          # 单位
  dpi = 100)
